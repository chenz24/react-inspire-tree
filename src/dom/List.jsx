...
export default class List extends React.Component {
  ...
  shouldComponentUpdate(nextProps) {
    return find(nextProps.nodes, 'itree.dirty') || stateComparator(this.props, nextProps);
  }
  ...
  render() {
    ...
    const items = renderNodes.map(
      node => <ListItem dom={this.props.dom} key={node.id} node={node} />);
    ...
    return <ol className="tree-list">{items}{this.props.children}</ol>;
  }
}
