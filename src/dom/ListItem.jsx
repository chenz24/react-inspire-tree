...
export default class ListItem extends React.Component {
  ...
  onDragStart(event) {
    ...
    if (this.props.dom.config.dragAndDrop.validateOn === 'dragstart') {
      ...
      const recursor = function(obj, iteratee) { // eslint-disable-line
  ...
  renderToggle() {
    const { node, dom } = this.props;
    const hasVisibleChildren = !dom.isDynamic ? node.hasVisibleChildren() : Boolean(node.children);
  ...
  render() {
    ...
    const li = (
      <li
        {...this.getAttributes()}
        ref={domNode => this.node = this.props.node.itree.ref = domNode} // eslint-disable-line
      >
