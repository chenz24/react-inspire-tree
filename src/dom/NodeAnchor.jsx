...
class NodeAnchor extends React.Component {
  ...
  click(event) {
    ...
    const handler = () => {
      ...
      if (event.shiftKey) {
        ...
        if (selected) {
          dom._tree.selectBetween.apply(dom._tree, dom._tree.boundingNodes(selected, node)); // eslint-disable-line
        }
      }
  ...
  render() {
    ...
    if (!dom.config.showCheckboxes) {
      ...
      const nodeIcon = node.itree.icon || (hasOrWillHaveChildren ? folderIcon : 'icon-file-empty');
